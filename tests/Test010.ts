...
export class Test010 extends TestCaseView {
    ...
    testCase(): void {
        try {
            ...
            const fluency = new BoundComponent(basicViewModel, { innerHtml: 'Hellooo, <i-v>name</i-v>' }).appendToParent(this.testArea);
            ...
            assert(fluency.className === '', 'Simon didnt say (update not true)');
            fluency.addAttributeMapping('bogus', 'cssClass', true);
            assert(fluency.className === 'does-nothing', 'Update called forces re-render');
            ...
            assert(fluency.content.style.display === 'none', 'setVisibility() hides');
            fluency.setVisibility('truthiness', false, true);
            assert(fluency.content.style.display === '', 'setVisibility() shows');
            fluency.addAttributeMapping('style', 'bold', true);
            assert(fluency.style.cssText === 'font-weight: bold;', 'addAttribute() changes attribute');
            ...
            fluency.render();
            assert(fluency.style.cssText === 'font-weight: bold;', 'addAttribute() regenerated during each rendering');
            fluency.content.removeAttribute('style');
            fluency.removeAttributeMapping('style', true);
            assert(fluency.style.cssText === '', 'removeAttribute() takes dynamic attribute away');

            fluency.setCssClass('cssClasses', true);
            assert(fluency.className === 'class-1 class-2', 'setCssClass() sets className');
            fluency.setCssStyle('bold', true);
            assert(fluency.style.cssText === 'font-weight: bold;', 'setCssStyle() changes style');
            fluency.addCssClassSwitch('some-class', 'truthiness', false, true);
            assert(fluency.className === 'class-1 class-2 some-class', 'addCssClassSwitch() adds class if property is true');

            // This case is a pain to test without bringing observables into it
            basicViewModel.truthiness = false;
            fluency.render();
            assert(fluency.className === 'class-1 class-2', 'class switch still active');
            fluency.removeCssClassSwitch('some-class');
            basicViewModel.truthiness = true;
            fluency.render();
            assert(fluency.className === 'class-1 class-2', 'class switch not active');
            ...
            assert(fluency.innerHTML === '<i-v noescape="">Hello <em>World</em></i-v>', 'setHtmlTemplate() sets noescape');
            fluency.setTextTemplate('rawHtml');
            assert(fluency.innerHTML === '<i-v>Hello &lt;em&gt;World&lt;/em&gt;</i-v>', 'setTextTemplate() doesnt set noescape');
            fluency.setTemplate('Hiya <i-v>name</i-v>!');
            assert(fluency.innerHTML === 'Hiya <i-v>World</i-v>!', 'setTemplate() acts the same as if original innerHTML were set');

            const fluentInput = new BoundComponent(basicViewModel, { type: 'input', attributes: { style: 'display: block;', i5_input: 'writable' } }).appendToParent(this.testArea);
            fluentInput.setValueAttribute('name', true);
            assert(fluentInput.value === 'World', 'setValueAttribute() sets a value mapping');
            ...
            assert(basicViewModel.writable2 === 'Input Test 1', 'addWriteTarget() adds a form input binding');

            fluentInput.removeWriteTarget('writable2');
            fluentInput.value = 'Input Test 2';
            fluentInput.content.dispatchEvent(new Event('input', { bubbles: true }));
            assert(basicViewModel.writable2 === 'Input Test 1', 'removeWriteTarget() removes the input binding');
